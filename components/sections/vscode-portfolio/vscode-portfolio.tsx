...
const I = {
  ...
  debug: (p: React.SVGProps<SVGSVGElement>) => (
    <svg viewBox="0 0 16 16" fill="none" {...p}>
  ...
  ext: (p: React.SVGProps<SVGSVGElement>) => (
    // Extensions (pinwheel of squares)
    <svg viewBox="0 0 16 16" fill="none" {...p}>
      <rect x="6.2" y="1" width="3.6" height="3.6" rx="0.8" transform="rotate(45 8 2.8)" fill="currentColor"/>
      <rect x="6.2" y="11.4" width="3.6" height="3.6" rx="0.8" transform="rotate(45 8 13.2)" fill="currentColor" fillOpacity=".85"/>
      <rect x="1" y="6.2" width="3.6" height="3.6" rx="0.8" transform="rotate(45 2.8 8)" fill="currentColor" fillOpacity=".7"/>
      <rect x="11.4" y="6.2" width="3.6" height="3.6" rx="0.8" transform="rotate(45 13.2 8)" fill="currentColor" fillOpacity=".55"/>
    </svg>
  ),
  run: (p: React.SVGProps<SVGSVGElement>) => (
    // Run & Debug (play triangle with gear)
    <svg viewBox="0 0 16 16" fill="none" {...p}>
      <path d="M3.5 2.3v9.9a.6.6 0 0 0 .92.5l7.9-4.95a.6.6 0 0 0 0-1.02L4.42 1.78a.6.6 0 0 0-.92.5Z" fill="currentColor"/>
      <circle cx="12.2" cy="12.2" r="2.6" fill="#0a0a0a" stroke="currentColor" strokeWidth="1"/>
      <path d="M12.2 10.6v.5M12.2 12.9v.5M10.6 12.2h.5M13.3 12.2h.5M11 11l.35.35M13.05 13.05l.35.35M11 13.4l.35-.35M13.05 11.35l.35-.35" stroke="currentColor" strokeWidth="0.9" strokeLinecap="round"/>
    </svg>
  ),
  remote: (p: React.SVGProps<SVGSVGElement>) => (
    // Remote / live preview (monitor with status dot)
    <svg viewBox="0 0 16 16" fill="none" {...p}>
      <rect x="1.5" y="2.5" width="13" height="8.5" rx="1.2" stroke="currentColor" strokeWidth="1.15"/>
      <path d="M6 13.5h4M8 11v2.5" stroke="currentColor" strokeWidth="1.15" strokeLinecap="round"/>
      <circle cx="12.3" cy="9.2" r="2" fill="#0a0a0a" stroke="currentColor" strokeWidth="1"/>
      <circle cx="12.3" cy="9.2" r="0.55" fill="currentColor"/>
    </svg>
  ),
  ...
  ext_arrow: (p: React.SVGProps<SVGSVGElement>) => (
    <svg viewBox="0 0 16 16" fill="none" {...p}>
...
const DEVELOPER_CODE_LINES = [
  "/** Thanh Tri — Business Analyst & Full Stack Developer */",
  "interface Developer {",
  "  name: string;",
  "  role: string;",
  "  skills: string[];",
  ...
  '  name: "Thanh Tri",',
  '  role: "Business Analyst · Full Stack Developer · UI/UX Designer",',
  '  skills: ["React", "Next.js", "FastAPI", "Node.js", "C#", "AWS"],',
  '  focus: "Shipping production-grade AI systems",',
  '  passion: "Machine learning technology & server service business",',
  "};",
  "",
  "// Let's build something extraordinary",
];

/* ---------- File tree data (mirrors the real thanhtri.ba repo layout) ---------- */
const fileStructure: FileNode[] = [
  { name: ".next", type: "folder", children: [], defaultOpen: false, muted: true },
  {
    name: "app",
    type: "folder",
    defaultOpen: true,
    children: [
      ...
      { name: "icon.png", type: "file", extension: "png" },
      { name: "layout.tsx", type: "file", extension: "tsx" },
      { name: "page.tsx", type: "file", extension: "tsx", gitStatus: "M" },
    ],
  },
  {
    name: "components",
    ...
    children: [
      ...
      },
      {
        ...
        defaultOpen: false,
        children: [
          { name: "LightRays.tsx", type: "file", extension: "tsx" },
          { name: "ProfileCard.tsx", type: "file", extension: "tsx" },
        ],
      },
      {
        name: "sections",
  ...
  },
  {
    ...
    type: "folder",
    defaultOpen: false,
    children: [{ name: "index.ts", type: "file", extension: "ts" }],
  },
  {
    name: "hooks",
    type: "folder",
  ...
  { name: "node_modules", type: "folder", children: [], defaultOpen: false, muted: true },
  {
    name: "public",
    type: "folder",
    defaultOpen: false,
  ...
  { name: "components.json", type: "file", extension: "json" },
  { name: "eslint.config.mjs", type: "file" },
  { name: "next-env.d.ts", type: "file" },
  { name: "next.config.ts", type: "file", extension: "ts" },
  { name: "package-lock.json", type: "file", extension: "json", gitStatus: "U" },
  ...
  { name: "README.md", type: "file", extension: "md", gitStatus: "M" },
];
...
const FILE_CONTENTS: Record<string, { code: string; lang: string }> = {
  "Hero.tsx": {
    lang: "TypeScript JSX",
    code: `import { FrameToFullscreen } from "@/components/animations/FrameToFullscreen";
    ...
    <FrameToFullscreen
      titleComponent={
        <h1 className="text-5xl font-semibold text-white">
          Designing Systems That Power
          <ScrollFloat textClassName="text-white font-bold">
            Better Decisions
          </ScrollFloat>
        </h1>
      }
...
export default function Home() {
  ...
  const bio = {
    ...
    focus: "Crafting digital experiences with Three.js, GSAP & Next.js",
  };
  ...
  { name: "ProofStack", tag: "Developer Trust Platform", score: "99.4%" },
  { name: "KodaiRateIQ", tag: "AI Rate Intelligence", score: "Realtime" },
  { name: "CyberShield India", tag: "Digital Forensics & Blockchain", score: "Polygon" }
];`,
  },
  "layout.tsx": {
    lang: "TypeScript JSX",
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <nav className="fixed top-0 inset-x-0 z-50 backdrop-blur-md bg-black/40 border-b border-white/10 px-6 py-4 flex items-center justify-between">
      <span className="font-bold tracking-tight text-white">ThanhTri</span>
  ...
  "ProjectCard.tsx": {
    lang: "TypeScript JSX",
    code: `export function ProjectCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="p-6 rounded-2xl bg-neutral-900 border border-neutral-800 hover:border-purple-500/50 transition duration-300">
  ...
  return (
    <footer className="py-12 border-t border-neutral-900 text-center text-xs text-neutral-500 font-mono">
      © 2026 ThanhTri — Crafted with Intent
    </footer>
  );
}`,
  },
...
export { gsap, ScrollTrigger };`,
  ...
  "transitions.ts": {
    lang: "TypeScript",
    code: `export class PageTransition {
  static ease = [0.22, 1, 0.36, 1];
  static duration = 0.8;
}`,
  },
  "utils.ts": {
...
export function cn(...inputs: ClassValue[]) {
  ...
  name: "ThanhTri",
  title: "Creative Developer & Systems Engineer",
  url: "https://thanhtri.ba",
};`,
  },
  "logo.svg": {
    lang: "SVG",
    code: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none">
  <path d="M20 20L80 50L20 80Z" fill="url(#grad)" />
  <defs>
  ...
  "hero.png": {
    lang: "Image Asset",
    code: `// [Binary Image Asset: hero.png]
// Resolution: 1920x1080 | Format: PNG (High Color Depth)
// Optimized asset preloaded by AssetPreloader pipeline.`,
  },
  "favicon.svg": {
    lang: "SVG",
    code: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
  <rect width="32" height="32" rx="8" fill="#a78bfa" />
  <text x="8" y="22" fill="#fff" font-weight="bold" font-size="18">T</text>
</svg>`,
  },
  "globals.css": {
    lang: "CSS",
...
@layer base {
  ...
  "README.md": {
    lang: "Markdown",
    code: `# Portfolio — ThanhTri

Cinematic digital experiences crafted with Three.js, GSAP, and Next.js.

## Features
- VS Code Interactive Terminal & File Explorer
- 3D Spline Interactive Scenes
- Parallax & Fluid Motion Transitions
- Optimized Asset Loading & Sound Feedback`,
  },
};

export const VSCodePortfolio: React.FC = () => {
  const [activeProject, setActiveProject] = React.useState<Project | null>(null);
  const [openTabs, setOpenTabs] = React.useState<string[]>(["Hero.tsx"]);
  const [activeFile, setActiveFile] = React.useState<string>("Hero.tsx");

  const handleSelectFile = (node: FileNode) => {
    if (!openTabs.includes(node.name)) {
      setOpenTabs((prev) => [...prev, node.name]);
    }
  ...
  const handleCloseTab = (fileName: string, e: React.MouseEvent) => {
    ...
    if (activeFile === fileName) {
      setActiveFile(nextTabs[nextTabs.length - 1] || "Hero.tsx");
    }
  };
  ...
  return (
    <div className="w-full h-full bg-[#0a0a0a] text-[#cccccc] flex flex-col overflow-hidden rounded-[6px]"
      ...
      <div className="flex flex-1 min-h-0">
        {/* Activity bar */}
        <div className="w-[44px] bg-[#0a0a0a] border-r border-[#1a1a1a] flex flex-col items-center py-2 shrink-0 text-[#8a8a8a]" style={{ gap: "4px" }}>
          <div className="w-8 h-8 rounded-[6px] bg-white/[0.08] flex items-center justify-center text-white">
            <I.files className="w-[18px] h-[18px]" />
          </div>
          <div className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
            <I.search className="w-[18px] h-[18px]" />
          </div>
          <div className="relative w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
            <I.git className="w-[18px] h-[18px]" />
            <span className="absolute -bottom-0.5 -right-0.5 min-w-[15px] h-[15px] px-[3px] rounded-full bg-[#3b82f6] text-white text-[8.5px] font-semibold flex items-center justify-center leading-none">
              196
            </span>
          </div>
          <div className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
            <I.run className="w-[18px] h-[18px]" />
          </div>
          <div className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
            <I.remote className="w-[18px] h-[18px]" />
          </div>
          <div className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
            <I.ext className="w-[18px] h-[18px]" />
          </div>
          <div className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
            <I.beaker className="w-[18px] h-[18px]" />
          </div>
        ...
        <div className="w-[210px] bg-[#0a0a0a] border-r border-[#1a1a1a] shrink-0 flex flex-col">
          ...
          <div className="px-2 flex-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <div className="flex items-center px-1 py-[3px] text-[11px] text-[#cfcfcf] font-medium">
              ...
              <span>THANHTRI.BA</span>
            </div>
            <FileTree
              data={fileStructure}
              defaultOpen={false}
              initialActive={activeFile}
        ...
        <div className="flex-1 flex min-w-0 bg-[#0d0d0d]">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            {/* Tab bar */}
            <div className="h-[32px] bg-[#0a0a0a] border-b border-[#1a1a1a] flex items-center text-[11px] shrink-0 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {openTabs.map((fileName) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "h-full flex items-center gap-2 px-3.5 border-r border-[#1a1a1a] cursor-pointer select-none transition-colors",
                      isActive
                        ? "bg-[#0d0d0d] text-white border-t-2 border-t-[#a78bfa]"
                        : "bg-[#080808] text-[#888888] hover:bg-[#0c0c0c] hover:text-[#cccccc]"
                    )}
                  >
                    <span className="font-mono text-[11px]">{fileName}</span>
                    <span
                      onClick={(e) => handleCloseTab(fileName, e)}
                      className="ml-1 text-[#7a7a7a] hover:text-white rounded px-1 hover:bg-white/10"
                    >
                      ×
                    </span>
                  </div>
            ...
            {/* Editor body */}
            {activeFile !== "Hero.tsx" && FILE_CONTENTS[activeFile] ? (
              <div className="flex-1 overflow-auto p-8 font-mono text-[13px] leading-[1.6] bg-[#0d0d0d] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                <div className="mb-6 text-[11px] text-[#6a6a6a] flex items-center gap-2 select-none border-b border-white/[0.05] pb-3">
                  <span className="text-[#a78bfa] font-bold">THANHTRI.BA</span>
                  <span>/</span>
                  <span className="text-white font-medium">{activeFile}</span>
                  <span className="ml-auto text-[10px] text-[#555555] uppercase tracking-wider">{FILE_CONTENTS[activeFile].lang}</span>
                </div>
                <div className="space-y-1">
                  {FILE_CONTENTS[activeFile].code.split("\n").map((line, idx) => (
                    <CodeLine key={idx} n={idx + 1} line={line} />
                  ))}
                </div>
              </div>
            ) : (
            <div className="flex-1 overflow-auto px-20 pt-20 pb-16 min-w-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              <Safari_01
                url="thanhtri.dev/developer.ts"
                copyText={DEVELOPER_CODE_LINES.join("\n")}
                className="mb-12 max-w-[820px]"
                style={{ marginLeft: "24px", marginRight: "24px", marginBottom: "24px" }}
              >
                <div className="relative">
                  <span className="absolute top-4 right-6 flex items-center gap-1.5 text-[10px] tracking-[0.1em] text-[#5a5a5a] uppercase select-none">
                    <span className="w-3.5 h-3.5 rounded-[3px] bg-[#3178c6] text-white flex items-center justify-center text-[7px] font-bold font-mono">TS</span>
                    TypeScript
                  </span>
                  <pre className="px-10 py-9 leading-[2.1] tracking-[0.01em] overflow-auto font-mono text-[13.5px] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" style={{ maxHeight: "420px" }}>
{DEVELOPER_CODE_LINES.map((line, i) => (
  <CodeLine key={i} n={i+1} line={line} />
))}
                    <div className="flex items-baseline">
                      <span className="inline-block w-10 shrink-0" />
                      <span className="inline-block w-[2px] h-[15px] bg-[#c4b5fd] animate-pulse" aria-hidden />
                    </div>
                  </pre>
                </div>
              </Safari_01>
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: "44px" }}>
              ...
              <section>
                ...
                <div
                  className="relative rounded-[18px] overflow-hidden isolate"
                  style={{
                    height: "420px",
                    ...
                    // Force its own compositing/stacking context so the ScrollStack
                    // children's imperative transforms never briefly render outside
                    // this frame while an ancestor (FrameToFullscreen) is mid pin/scale.
      ...
      <div className="h-[26px] bg-[#0a0a0a] border-t border-[#1a1a1a] flex items-center text-[11px] text-[#bdbdbd] shrink-0">
        ...
        <div className="ml-auto flex items-center gap-5 px-4 text-[#9a9a9a]">
          ...
          <span>UTF-8</span>
          <span>LF</span>
          <span className="flex items-center gap-1">{"{ }"} {FILE_CONTENTS[activeFile]?.lang || "TypeScript JSX"}</span>
...
function tokenize(line: string): React.ReactNode {
  ...
  const regex = new RegExp(
    '("[^"]*"|\'[^\']*\'|`[^`]*`|\\b(import|export|from|default|function|const|let|var|return|interface|type|class|extends|implements|as|true|false|null|undefined|async|await|script|build|dev)\\b|\\b[A-Z][a-zA-Z0-9_]*\\b|\\/?>|<[a-zA-Z0-9_]+|[{}[\\](),;:.]|[=+\\-*/])',
    "g"
  );
  let last = 0; let k = 0; let m: RegExpExecArray | null;
  while ((m = regex.exec(line))) {
    if (m.index > last) push(<span className="text-[#d4d4d4]">{line.slice(last, m.index)}</span>, k++);
    const tok = m[0];
    let cls = "text-[#d4d4d4]";
    if (tok.startsWith('"') || tok.startsWith("'") || tok.startsWith("`")) cls = "text-[#ce9178]";
    else if (["import","export","from","default","function","const","let","var","return","interface","type","class","extends","implements","as","true","false","null","undefined","async","await"].includes(tok)) cls = "text-[#c586c0]";
    else if (/^[A-Z]/.test(tok)) cls = "text-[#4ec9b0]";
    else if (tok.startsWith("<")) cls = "text-[#569cd6]";
    else if (tok === "/>" || tok === ">") cls = "text-[#808080]";
    else if ("{}[]()".includes(tok)) cls = "text-[#ffd700]";
    else if (",;:.".includes(tok)) cls = "text-[#9a9a9a]";
    push(<span className={cls}>{tok}</span>, k++);
    last = m.index + tok.length;
  }
  if (last < line.length) push(<span className="text-[#d4d4d4]">{line.slice(last)}</span>, k++);
  return parts;
}
