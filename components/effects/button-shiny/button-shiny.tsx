...
interface ButtonCtaProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  /** "primary" (default) gets the animated comet treatment; "secondary" is a quieter glass/outline style. */
...
function ButtonCta({
  ...
  if (variant === "secondary") {
    return (
      <button
        className={cn(
          "group relative inline-flex items-center justify-center h-12 px-8 rounded-[11px] cursor-pointer outline-none leading-none",
          "border border-white/[0.12] bg-white/[0.02] backdrop-blur-sm",
          "transition-[background-color,border-color,transform] duration-300 ease-out",
          "hover:bg-white/[0.06] hover:border-[#a78bfa]/40",
          "active:translate-y-px",
          "focus-visible:ring-2 focus-visible:ring-[#a78bfa]/40 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0a]",
          className
        )}
        {...props}
      >
        <span className="relative flex items-center justify-center gap-2 text-[15px] font-light tracking-[0.01em] whitespace-nowrap text-[#cfcfcf] transition-colors duration-300 group-hover:text-white">
          {children ?? label}
        </span>
      </button>
    );
